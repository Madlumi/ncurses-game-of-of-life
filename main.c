...
int main(int argc, char *argv[]){
   //Init
   WINDOW *win;cell **area;cell **areaT;int startx, starty, width, height;int ch;
   //ncuses init
   init();
   square sq={.x=0,.y=0,.w=COLS-3,.h=LINES-2};
   sq.x=(COLS - sq.w )/2;sq.y=(LINES- sq.h )/2;
   area= (cell **)calloc(sq.w, sizeof(cell *)); 
   for(int i = 0;i < sq.w; ++i){area[i] = (cell *)calloc(sq.h, sizeof(cell));}

   areaT= (cell **)calloc(sq.w, sizeof(cell *)); 
   for(int i = 0;i < sq.w; ++i){areaT[i] = (cell *)calloc(sq.h, sizeof(cell));}
   refresh();
   ...
   for (int yy = 1; yy < sq.h-1; yy++){
      for (int xx = 1; xx < sq.w; xx++){
         area[xx][yy].data=rand()%10<2;   
   }}
   while(getch() != KEY_F(1)){
